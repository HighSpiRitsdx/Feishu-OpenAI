    ...
    if (OpenAISecret) {
      ...
      try {
        ...
        if (err.status === 401)
          return {
            error:
              'OpenAI ChatGPT 返回 401 错误，应该是你配置错了 OpenAI 平台的 Secret Token，请重新配置。',
          };
        if (err.status === 429)
          return {
            error:
              'OpenAI ChatGPT 的 API 返回 429 错误，可能是你的额度已耗尽，你需要去 OpenAI 付费充值你的 API 配额。付费购买的 ChatGPT Plus 仅是网页版，API 需要另外充值。',
          };
        return err.message;
      }
    }
